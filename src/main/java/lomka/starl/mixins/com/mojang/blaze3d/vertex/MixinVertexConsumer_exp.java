...
public interface MixinVertexConsumer_exp {
    ...
    *///?}
     ...
     *         normalization when pose.trustedNormals is true. Descriptor is stable across all
     *         supported versions (1.21.6+), so no per-version gating is needed.
     */
    @Overwrite
    ...
    *///?} else if >=1.21.11 && <26.1 {
    /**
     * @author Starlev
     * @reason Zero-allocation bulk quad transfer: vanilla allocates one normal Vector3f and four position
     *         Vector3f per quad (5 objects). Direct BakedQuad record accessors + scalar transforms emit the
     *         same vertices with no heap traffic. Face normal uses Direction step components through the pose
     *         normal matrix (equivalent to transformNormal(getUnitVec3f()) without the Vector3f allocation).
     *         1.21.6/1.21.9 BakedQuad is an int[8] record without position()/packedUV(), and 26.1+ removed
     *         putBulkData (putBakedQuad/QuadInstance) — hence the >=1.21.11 && <26.1 gate.
     */
    @Overwrite
    default void putBulkData(PoseStack.Pose pose, BakedQuad quad, float r, float g, float b, float a, int light, int overlay) {
        Direction direction = quad.direction();
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            float vy = Math.fma(poseMat.m01(), px, Math.fma(poseMat.m11(), py, Math.fma(poseMat.m21(), pz, poseMat.m31())));
            float vz = Math.fma(poseMat.m02(), px, Math.fma(poseMat.m12(), py, Math.fma(poseMat.m22(), pz, poseMat.m32())));
            this.addVertex(vx, vy, vz, color, u, v, overlay, lightWithEmission, nx, ny, nz);
        }
    }
     ...
     *         trustedNormals fast-path + direct int[] vertex array access.
     */
    @Overwrite
