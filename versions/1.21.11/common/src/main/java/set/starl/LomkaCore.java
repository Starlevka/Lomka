...
public class LomkaCore {
    ...
    public static final ThreadLocal<Boolean> IN_TEXTURE_DECODE = new ThreadLocal<>();
    ...
    private static Executor lomka$createTextureDecodeExecutor() {
        // Virtual threads are ideal for texture decode: each decode is an
        // independent I/O-bound task. Virtual threads don't block platform
        // threads during I/O and scale automatically. Available since Java 21.
        return Executors.newVirtualThreadPerTaskExecutor();
    }

    private static void lomka$logOptionalJvmOptimizations() {
        if (JAVA_25_PLUS) {
            LOGGER.info("Java 25+ detected.");
            if (CONFIG.java.java25Optimizations) {
                LOGGER.info("Java 25 optimizations ENABLED.");
                lomka$logAotCacheStatus();
                lomka$logCompactHeadersStatus();
            } else {
    ...
    private static void lomka$logJavaOptimizations() {
        ...
        LOGGER.info("  Virtual threads: ENABLED (texture decode executor)");
        LOGGER.info("  Math.fma: ENABLED (lerp, vector math, distSq)");
        LOGGER.info("  Generation-based cache: ENABLED (no clear() per frame)");
        LOGGER.info("  Avalanche hash: ENABLED (uniform section cache distribution)");
        ...
        if (JAVA_25_PLUS) {
            ...
            LOGGER.info("  C2 auto-vectorization: ENABLED (automatic, faster Math.min/max/distSq)");
	...
	public static final class MathHelper {
        ...
        // Batch distance² — C2 auto-vectorizes on Java 25+
        ...
        // Fast hash for section keys — avalanche for uniform distribution
